...
import {Config, Artifact, ArtifactResponse, IDivSrcCore, IdentityFields} from './interfaces'
...
class DivSrcCore implements IDivSrcCore {
  ...
  public async injectSystemJs() {
    await this.striptInjector(this.config?.systemJsUrl || 'https://divsource.s3.eu-central-1.amazonaws.com/libs/system.js', 'systemjs')
    await this.striptInjector(this.config?.systemJsExtrasNamed || 'https://unpkg.com/systemjs/dist/extras/named-register.js', 'systemjs_extras_named')
    await this.striptInjector(this.config?.systemJsExtrasAmd || 'https://unpkg.com/systemjs/dist/extras/amd.js', 'systemjs_extras_amd')
  }
  ...
  public async mountArtifact(artifact: Artifact) {
    ...
    if (this.registry[artifact.installationId] && this.registry[artifact.installationId].isActive) {
      common.log(`Mount From Cache: ${artifact.zone}/${artifact.installationId}`)
      return Promise.resolve(null)
    }

    common.log(`Mount From HTTP Request: ${artifact.zone}/${artifact.installationId}`)
    const injector = window['System'] ? this.es6ModuleImporter(artifact) : this.tagScriptImporter(artifact)
    return injector.then((_artifactObj: any) => {
      _artifactObj = _artifactObj.default || _artifactObj
      if (_artifactObj) {
        this.artifacts.filter(f => f.zone === artifact.zone).forEach(ar => {
          if (this.registry[ar.installationId]) {
            this.registry[ar.installationId].isActive = false
          }
        });
        this.registry[artifact.installationId] = {...artifact, isActive: true}
        delete window[artifact.artifactId]
        this.installations[artifact.installationId] = _artifactObj.default || _artifactObj
      }
    }).catch((err) => {
      console.log(err)
      common.error(`failed to fetch app for zone: ${artifact.zone}, version: ${artifact.version}. ${err.message}`)
      this.registry[artifact.installationId] = {error: 'Failed to mount ' + err.message ? err.message : ''}
    })
  }
  ...
  public getArtifactByInstallationId(installationId: string) {
    ...
    const overrideKey = Object.keys(overrides).find(zone => overrides[zone].installationId === installationId)
    const override = overrideKey && overrides[overrideKey]

    if (this.artifacts) {
      const artifact = this.artifacts.find(f => f.installationId === installationId)
      if (override) {
        return {
          ...artifact,
          ...override
  ...
  public setZoneLocalOverride(zone: string, artifactId: string, version: string, fileName?: string, baseUrl?: string, installationId?: string) {
    let obj: Artifact = {zone, artifactId, version, installationId: `installation_${this.generateId()}`}
    if (fileName) {
  ...
  public getArtifactByZone(zone: string): Artifact | undefined {
    const list: Artifact[] = (this.artifacts || []).filter(a => a.zone === zone)
    const resolvedOverride: Artifact | undefined = this.tryFindArtifactOverrideByZone(zone)
    const resolvedArtifact: Artifact | undefined = list && list[0]
    if (resolvedOverride) {
      common.log(`Applying local override for zone "${zone}" with params: ${JSON.stringify(resolvedArtifact)}`)
      return {
        ...resolvedArtifact,
        ...resolvedOverride
      }
  ...
  public async init(config: Config): Promise<any> {
    ...
    try {
      this.debug = localStorage && (!!localStorage.getItem('_debug') || !!localStorage.getItem('divsrc_debug'))
      this.config = config
      const response: ArtifactResponse = this.mockedResponse || await this.fetchMap()
      ...
      if (config.emulators && config.emulators.length > 0) {
        const emulatedResults = await Promise.all(config.emulators.map(em => axios.get(em.url).then(r => r && r.data).catch(err => {
          console.log(err)
          return {}
        })))
        ...
        this.artifacts = this.artifacts.map(ar => {
          ...
          if (mappedEmulatedResults[ar.artifactId] && (rule === undefined || rule === true || ruleIsObject)) {
  ...
  public cleanIdentityChange(callback: void) {
    this.identityChangeHandlers = this.identityChangeHandlers.filter(clb => clb !== callback)
  }
}
